> This document is an adapted calque of the [Tiger Style](https://github.com/tigerbeetle/tigerbeetle/blob/1e40c4c876216b4e27d70fa2c45adb47124e2a7b/docs/TIGER_STYLE.md). It is translated from Zig to C with additional stuff included.
...
RocketStyle's coding style is evolving. A collective give-and-take at the intersection of
...
In other words, RocketStyle has a “zero technical debt” policy. We do it right the first time. This
...
- Use explicitly-sized types like `uint32_t` for everything, avoid architecture-specific `size_t`.
...
- Be explicit. Minimize dependence on the compiler to do the right thing for you.
  ...
  In particular, extract hot loops into stand-alone functions with primitive arguments instead of
  struct pointers (see [an example](https://github.com/tigerbeetle/tigerbeetle/blob/0.16.19/src/lsm/compaction.zig#L1932-L1937)).
...
- Use `snake_case` for function, variable, and file names. The underscore is the closest thing we
  ...
  don't use `CamelCase` style for "struct" files to keep the convention simple and
  consistent.
...
- For the rest, follow the project's existing conventions.
...
- Infuse names with meaning. For example, `Allocator allocator` is a good, if boring name,
  but `Allocator gpa` and `Allocator arena` are excellent. They inform the reader whether
  a cleanup function like `allocator_deinit()` should be called explicitly.
...
- _Order_ matters for readability (even if it doesn't affect semantics). On the first read, a file
  ...
  int tracer_init(tracer *tracer, allocator *gpa, time time) {
      ...
  }
  ```
...
- Don't overload names with multiple meanings that are context-dependent. For example, RocketStyle
...
- Named arguments can be approximated through designated initializer structs. Use this when
  arguments can be mixed up. A function taking two `uint64_t` must use a struct parameter. If
  an argument can be `NULL`, it should be named so that the meaning of `NULL` literal at the
  call site is clear.
...
- If you don't mean a function argument to be copied when passed by value, and if the argument type
  is more than 16 bytes, then pass the argument as `const T *`. This will catch bugs where the caller
...
- Construct larger structs _in-place_ by passing an _out pointer_ during initialization.
  ...
  **Prefer:**
  ...
  ```c
  int large_struct_init(struct LargeStruct *target) {
    *target = (struct LargeStruct){
  ...
  int main(void) {
    struct large_struct target;
    if (large_struct_init(&target) < 0) {
      return -1;
    }
  }
  ```
  ...
  **Over:**
  ...
  ```c
  struct large_struct large_struct_init(void) {
    return (struct LargeStruct){
      // moving the initialized object.
    };
  }

  int main(void) {
    struct large_struct target = large_struct_init();
  }
  ```
...
- Use simpler function signatures and return types to reduce dimensionality at the call site, the
  ...
  `bool` trumps `uint64_t`, `uint64_t` trumps a struct with a validity flag, and a struct
  with a validity flag trumps an error-returning function.
...
- Be on your guard for **[buffer bleeds](https://en.wikipedia.org/wiki/Heartbleed)**. This is a
  ...
  deterministic guarantees as required by RocketStyle to be violated.

- Use newlines to **group resource allocation and deallocation**, i.e. before the resource
  allocation and after the corresponding cleanup section, to make leaks easier to spot.
...
- Show your intent with respect to division. For example, use `a / b` with an assertion for exact
  division, or `(a + b - 1) / b` for ceiling division to show the reader you've thought through
  all the interesting scenarios where rounding may be involved.
...
- Hard limit all line lengths, without exception, to at most 100 columns for a good typographic
  ...
  structure, add a trailing comma, close your eyes and let `clang-format` do the rest.
...
RocketStyle has **a “zero dependencies” policy**, apart from the C toolchain. Dependencies, in
...
specialized instruments each with a dedicated manual. Our primary tool is C. It may not be the
best for everything, but it's good enough for most things. We invest into our C tooling to ensure
...
For example, the next time you write a script, instead of `scripts/*.sh`, write `scripts/*.c`.
...
Standardizing on C for tooling is important to ensure that we reduce dimensionality, as the team,
...
At the end of the day, keep trying things out, have fun, and remember—it's called RocketStyle, not
