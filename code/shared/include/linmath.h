...
#define LINMATH_H_DEFINE_VEC(n) \
    ...
    LINMATH_H_FUNC void vec##n##_add( \
        vec##n r, \
        vec##n const a, \
        vec##n const b \
    ) { \
    ...
    LINMATH_H_FUNC void vec##n##_scale( \
        vec##n r, \
        vec##n const v, \
        float const s \
    ) { \
    ...
    LINMATH_H_FUNC void vec##n##_min( \
        vec##n r, \
        vec##n const a, \
        vec##n const b \
    ) { \
    ...
    LINMATH_H_FUNC void vec##n##_max( \
        vec##n r, \
        vec##n const a, \
        vec##n const b \
    ) { \
...
LINMATH_H_FUNC void
mat4x4_scale_aniso(mat4x4 M, mat4x4 const a, float x, float y, float z) {
...
LINMATH_H_FUNC void
mat4x4_translate_in_place(mat4x4 M, float x, float y, float z) {
    vec4 t = {x, y, z, 0};
    vec4 r;
    int i;
    for (i = 0; i < 4; ++i) {
...
LINMATH_H_FUNC void
mat4x4_from_vec3_mul_outer(mat4x4 M, vec3 const a, vec3 const b) {
    int i, j;
    for (i = 0; i < 4; ++i)
        for (j = 0; j < 4; ++j)
...
LINMATH_H_FUNC void mat4x4_invert(mat4x4 T, mat4x4 const M) {
    ...
    float idet = 1.0f
        / (s[0] * c[5] - s[1] * c[4] + s[2] * c[3] + s[3] * c[2] - s[4] * c[1]
           + s[5] * c[0]);
...
LINMATH_H_FUNC void
mat4x4_frustum(mat4x4 M, float l, float r, float b, float t, float n, float f) {
    M[0][0] = 2.f * n / (r - l);
...
LINMATH_H_FUNC void
mat4x4_look_at(mat4x4 m, vec3 const eye, vec3 const center, vec3 const up) {
